...
function PageCreateProfile() {
    ...
    const onSubmit = async (data) => {
        if (!validIdentifier.file) return toast({
            title: "Valid ID is required for verication",
            status: 'error',
        ...
        if (response.data) {
            ...
            await ticketController.createVerificationTicket(validIdentifier.file);
            return toast({
            ...
            <form onSubmit={handleSubmit(onSubmit)} autoComplete="off">
                ...
                <Box mt={"10"} >
                    ...
                    <Textarea
                        ...
                        disabled></Textarea>
                    <label htmlFor={"validIdentifierId"}  >
                        <Box backgroundColor={useColorModeValue("#D4D4D4", "#131720")} w={"full"} p={3} borderRadius={"md"} cursor={"pointer"}>
                            <Text color={useColorModeValue("gray", "gray.400")} textAlign={"center"} fontSize={"smaller"}>{validIdentifier.name}</Text>
                        </Box>
                    </label>
                    <input
                        type="file"
                        id="validIdentifierId"
                        ref={validIdentifierRef}
                        accept="image/png, image/jpeg"
                        hidden
                        onChange={onAddValidIdentifier} />
                </Box>
